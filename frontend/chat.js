...
window.addEventListener("cipherline:languagechange", () => {
  if (currentUser) {
    App.renderSidebar({ activePage: "chat", user: currentUser });
  }
  renderPeople();
  renderActiveChat();
  refreshSocketLabels();
...
window.addEventListener("visibilitychange", () => {
  if (canMarkActiveServerRead()) {
    void syncActiveServerReadState();
  }
});
...
async function bootstrap() {
  ...
  const activeServerId = App.getActiveServerId();
  if (!activeServerId) {
    window.location.href = "/servers";
    return;
  }

  await loadServerChat(activeServerId);
}

function renderPeople() {
  const people = (serverContext?.members || []).filter((member) => member.id !== currentUser.id);

  if (!people.length) {
    peopleList.innerHTML = `<div class="empty-state">${App.escapeHtml(App.t("state_no_people"))}</div>`;
    return;
  }
    ...
    <div class="member-item">
      <div class="member-profile-row">
        ...
        <div class="member-text-stack">
          <strong>${App.escapeHtml(person.username)}</strong>
          ${renderUserStatus(person)}
        </div>
      </div>
      <div class="member-item-row">
        <button class="member-action ghost" data-person-dm="${person.id}">${App.escapeHtml(App.t("button_message_user"))}</button>
        ${renderFriendAction(person)}
      </div>
    </div>
  `).join("");

  document.querySelectorAll("[data-person-dm]").forEach((button) => {
    button.addEventListener("click", () => {
      App.setActiveDirectMessageUserId(button.dataset.personDm);
      window.location.href = "/dms";
    });
  });

  document.querySelectorAll("[data-person-add]").forEach((button) => {
    button.addEventListener("click", async () => {
      const userId = button.dataset.personAdd;
      const username = button.dataset.personUsername || "";
...
function renderPersonAvatar(person) {
  if (person.avatar_url) {
    return `<img class="member-avatar" src="${App.escapeHtml(person.avatar_url)}" alt="${App.escapeHtml(person.username)}">`;
  }
  return `<div class="member-avatar-fallback">${App.escapeHtml(App.getInitials(person.username))}</div>`;
}
...
async function loadServerChat(serverId) {
  manuallyDisconnected = false;
  disconnectSocket();
  const payload = await App.apiFetch(`/servers/${serverId}`);
  serverContext = payload.server;
  activeChat = {
    type: "server",
    id: serverContext.id,
    name: serverContext.name,
    members: serverContext.members,
    memberCount: serverContext.member_count,
    messages: serverContext.messages,
    unreadCount: Number(serverContext.unread_count || 0),
  };
...
function renderFriendAction(person) {
  if (friendsState.friendIds.has(person.id)) {
    return `<button class="member-action ghost" type="button" disabled>${App.escapeHtml(App.t("state_friend_added"))}</button>`;
  }
  if (friendsState.outgoingIds.has(person.id)) {
    return `<button class="member-action ghost" type="button" disabled>${App.escapeHtml(App.t("state_request_sent"))}</button>`;
  }
  return `
    <button
      class="member-action ghost"
      type="button"
      data-person-add="${App.escapeHtml(person.id)}"
      data-person-username="${App.escapeHtml(person.username)}"
    >
      ${App.escapeHtml(App.t("button_add_friend"))}
    </button>
  `;
}
...
async function refreshFriendsState() {
  try {
    ...
    friendsState = {
      friendIds: new Set((payload.friends || []).map((item) => item.id)),
      outgoingIds: new Set((payload.outgoing_requests || []).map((item) => item.user?.id).filter(Boolean)),
    };
  } catch {
    friendsState = { friendIds: new Set(), outgoingIds: new Set() };
  }
}
...
function renderActiveChat() {
  ...
  clearComposerDrafts();
  activeContextEyebrow.textContent = App.t("state_active_server");
  ...
  if (!activeChat.messages.length) {
    messagesContainer.innerHTML = `<div class="empty-state">${App.escapeHtml(App.t("state_no_messages"))}</div>`;
  } else {
...
function connectSocket() {
  ...
  socket.addEventListener("message", (event) => {
    try {
      ...
      handleIncomingServerMessage(payload);
    } catch (error) {
...
function buildSocketUrl() {
  ...
  return `${base}/${encodeURIComponent(activeChat.id)}?token=${token}`;
}
...
function resolveMessageAvatarUrl(message, isOwn) {
  ...
  const member = serverContext?.members?.find((item) => item.id === message.user_id);
...
function handleIncomingServerMessage(message) {
  ...
  const isRemoteUserMessage = message.type === "message" && message.user_id !== currentUser.id;
  const shouldMarkReadNow = isRemoteUserMessage && canMarkActiveServerRead();

  if (activeChat && message.type === "message") {
    if (shouldMarkReadNow) {
      activeChat.unreadCount = 0;
    } else if (isRemoteUserMessage) {
      activeChat.unreadCount = Number(activeChat.unreadCount || 0) + 1;
      showServerMessageNotification(message);
    }
  }
...
function canMarkActiveServerRead() {
  return Boolean(activeChat) && document.visibilityState === "visible" && document.hasFocus();
}

async function syncActiveServerReadState() {
  if (!activeChat || !canMarkActiveServerRead()) {
    return;
  }
  try {
    await App.apiFetch(`/servers/${activeChat.id}/read`, { method: "POST" });
  } catch {
    return;
  }
  activeChat.unreadCount = 0;
...
function updateUnreadIndicators() {
  App.setDocumentTitle(Number(activeChat?.unreadCount || 0));
}

function showServerMessageNotification(message) {
  ...
  App.notify({
    title: `${message.username || App.t("state_active_server")} • ${activeChat?.name || "VyChat"}`,
    body,
    tag: `server:${activeChat?.id || "message"}`,
